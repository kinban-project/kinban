...
- Claim a message with `claim_next_assistant_message` when processing the message queue. Use the returned `message.id` to reply, release, defer, or complete that message; no short-lived context token is used.
- A manager message may authorise a write only when its `message.id` is supplied as `sourceMessageId`. MCP verifies that the sender is an active manager and that the group permission is enabled.
