...
- 直接の管理者タスクでは `sourceMessageId` と `claimId` は不要です。運営支援APIキーの発行者が有効な管理者であることと、日次または月次のグループ許可をMCPが確認します。メンバー問い合わせを起点にする場合だけ、現在claim中の管理者メッセージIDの `sourceMessageId` と同じ `claimId` を指定します。
