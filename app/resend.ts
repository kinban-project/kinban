function setting(name: string) { return process.env[name] ?? ""; }
...
export async function sendInvitationEmail(input: { to: string; token: string; expiresAt: string }) {
  ...
  const from = setting("RESEND_FROM_EMAIL") || "KINBAN <invite@kinban.jp>";
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({ from, to: [input.to], ...(setting("RESEND_REPLY_TO") ? { reply_to: setting("RESEND_REPLY_TO") } : {}), subject: "KINBANへの招待", text: `KINBANへの招待が届いています。\n\n${link}`, html: `<p>KINBANへの招待が届いています。</p><p><a href="${link}">KINBANを開始する</a></p><p>有効期限: ${input.expiresAt}</p>` }),
  });
