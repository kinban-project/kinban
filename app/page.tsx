...
export default function Home() {
  ...
  return <main className="shell"><button className="roster-launch" onClick={() => setShiftRosterOpen(true)}>シフト一覧</button>
    <aside className="demo-warning" role="note"><strong>デモ用サイトです</strong><p>予告なく終了する可能性があります。登録した予定や添付ファイルが漏洩する可能性もあります。</p><p>重要な情報は登録しないでください。</p></aside>
    {notice && <div className="upload-notice" role="alert">{notice}<button onClick={() => setNotice(null)} aria-label="閉じる">×</button></div>}
    <header className="topbar"><div className="brand"><span className="brand-mark">◒</span><span>My Day</span><span className="brand-pill">PRIVATE CALENDAR</span></div><div className="top-actions"><button className="ghost-button" onClick={() => setShiftOpen(true)}>シフト作成</button><button className="ghost-button" onClick={() => setGroupsOpen(true)}>グループ</button><a className="ghost-button" href="/api-guide">APIガイド</a><span className="sync-label">{process.env.NEXT_PUBLIC_LOCAL_MODE === "true" ? "ローカル開発モード" : userEmail ? "ChatGPTでログイン中" : "サンプル表示中"}</span>{userEmail ? <button className="ghost-button" onClick={() => setSettingsOpen(true)}>アカウント</button> : <a className="ghost-button" href="/signin-with-chatgpt?return_to=/">ChatGPTでログイン</a>}</div></header>
    <section className="intro"><div><p className="eyebrow">YOUR SPACE, YOUR RHYTHM</p><h1>今日を、少しずつ<br /><em>軽くする。</em></h1><p className="subcopy">予定とタスクをひとつに。あなたのペースで使える、静かなカレンダー。</p></div><button className="primary-button" onClick={() => openNew()}>＋ 予定を追加</button></section>
    ...
    {groupsOpen && <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setGroupsOpen(false); }}><div className="modal groups-modal"><GroupsPanel onChanged={() => { void loadCalendar(); }} /></div></div>}
    {shiftOpen && <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setShiftOpen(false); }}><div className="modal shift-modal"><ShiftBuilder /></div></div>}
