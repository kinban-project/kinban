...
function emptyDay(dayOfWeek: number): Day { return { dayOfWeek, status: "possible", startTime: "", endTime: "", note: "" }; }
function normalizeStatus(status: string): PreferenceStatus { return status === "want" || status === "off" || status === "unavailable" ? status : "possible"; }

export default function GroupPreferencesPanel({ groupId }: { groupId: string }) {
  const [days, setDays] = useState<Record<number, Day[]>>({});
  const [preference, setPreference] = useState<Preference>({ minDays: 0, maxDays: 7, minHours: 0, maxHours: 40, freeComment: "" });
  const [notice, setNotice] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    const timer = window.setTimeout(() => { void localApiFetch(`/api/groups/${groupId}/preferences`).then(async (response) => {
      if (!response.ok) return;
      const data = await response.json() as { preferences: Preference; availability: Array<Day & { status: string }> };
      const next: Record<number, Day[]> = {};
      for (const day of data.availability) (next[day.dayOfWeek] ??= []).push({ ...day, status: normalizeStatus(day.status) });
      setPreference({ minDays: data.preferences.minDays, maxDays: data.preferences.maxDays, minHours: data.preferences.minHours, maxHours: data.preferences.maxHours, freeComment: data.preferences.freeComment ?? "" });
      setDays(next);
    }); }, 0);
    return () => window.clearTimeout(timer);
  }, [groupId]);

  function rowsForState(current: Record<number, Day[]>, day: number) { return current[day]?.length ? current[day] : [emptyDay(day)]; }
  function rowsFor(day: number) { return rowsForState(days, day); }
  function updateDay(day: number, index: number, values: Partial<Day>) { setDays((current) => ({ ...current, [day]: rowsForState(current, day).map((row, rowIndex) => rowIndex === index ? { ...row, ...values } : row) })); }
  function addDay(day: number) { setDays((current) => ({ ...current, [day]: [...rowsForState(current, day), emptyDay(day)] })); }
  function removeDay(day: number, index: number) { setDays((current) => ({ ...current, [day]: rowsForState(current, day).filter((_, rowIndex) => rowIndex !== index) })); }
  async function save() {
    setSaving(true);
    const availability = labels.flatMap((_, day) => rowsFor(day));
    const response = await localApiFetch(`/api/groups/${groupId}/preferences`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...preference, availability }) });
    setNotice(response.ok ? "基本設定を保存しました" : ((await response.json().catch(() => ({})) as { error?: string }).error ?? "基本設定を保存できませんでした"));
    setSaving(false);
  }
