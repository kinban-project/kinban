...
type Log = { id: string; userEmail: string; action: string; entityType: string; summary: string; details?: string; createdAt: string };
...
const actionLabels: Record<string, string> = {
  ...
  "shift.delete": "シフト削除",
  "group.member": "メンバー変更",
  "group.member.status": "利用状態変更",
  "announcement.create": "お知らせ作成",
  "announcement.read": "お知らせ既読",
  "announcement.reply": "お知らせ返信",
};
...
export default function AuditLogPanel({ groupId }: { groupId: string }) {
  ...
  async function load() {
    ...
    } else setNotice("操作ログを取得できませんでした");
    setBusy(false);
  }
  ...
  return <section className="audit-panel">
    ...
    <form className="audit-filters" onSubmit={(event) => { event.preventDefault(); void load(); }}>
      ...
      <label>実行者<select value={userEmail} onChange={(event) => setUserEmail(event.target.value)}><option value="">すべて</option>{members.map((member) => <option key={member.userEmail} value={member.userEmail}>{nameMap.get(member.userEmail)}</option>)}</select></label>
      <label>操作<select value={action} onChange={(event) => setAction(event.target.value)}><option value="">すべて</option>{actionOptions.map((value) => <option key={value} value={value}>{actionLabels[value] ?? value}</option>)}</select></label>
      <label className="audit-search">内容検索<input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="概要で検索" /></label>
