...
type CustomSlot = { date: string; startTime: string; endTime: string; requiredCount: number; role: string };
function parseCustomSlots(input: unknown, planId: string, startDate: string, endDate: string): CustomSlot[] & Array<{ id: string; planId: string }> {
  const parsed = typeof input === "string" ? JSON.parse(input) as unknown : input;
  const rows = Array.isArray(parsed) ? parsed : parsed && typeof parsed === "object" && Array.isArray((parsed as { slots?: unknown }).slots) ? (parsed as { slots: unknown[] }).slots : null;
  if (!rows?.length) throw new Error("slots配列に1件以上の枠を指定してください");
  const used = new Set<string>();
  return rows.map((raw, index) => {
    if (!raw || typeof raw !== "object") throw new Error(`${index + 1}件目の枠が不正です`);
    const item = raw as Record<string, unknown>;
    const date = String(item.date ?? "");
    const startTime = String(item.startTime ?? "");
    const endTime = String(item.endTime ?? "");
    const role = String(item.role ?? "").trim().slice(0, 100);
    const requiredCount = Number(item.requiredCount);
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date) || date < startDate || date > endDate) throw new Error(`${index + 1}件目の日付が勤務枠の期間外です`);
    if (!/^\d{2}:\d{2}$/.test(startTime) || !/^\d{2}:\d{2}$/.test(endTime) || minutes(startTime) >= minutes(endTime) || minutes(startTime) % 30 !== 0 || minutes(endTime) % 30 !== 0) throw new Error(`${index + 1}件目の時刻は30分単位で、開始より終了を後にしてください`);
    if (!Number.isInteger(requiredCount) || requiredCount < 1 || requiredCount > 50) throw new Error(`${index + 1}件目の必要人数は1〜50の整数で指定してください`);
    const key = `${date}|${startTime}|${endTime}|${role}`;
    if (used.has(key)) throw new Error(`${index + 1}件目の枠が重複しています`);
    used.add(key);
    return { id: crypto.randomUUID(), planId, date, startTime, endTime, requiredCount, role };
...
export async function POST(request: Request) {
  const user = await getChatGPTUser();
  if (!user) return Response.json({ error: "ログインが必要です" }, { status: 401 });
  const body = await request.json() as { groupId?: string; name?: string; notes?: string; startDate?: string; endDate?: string; requestCloseDate?: string; openingTime?: string; closingTime?: string; slotMinutes?: number; requiredCount?: number; role?: string; slotRules?: Array<{ role?: string; requiredCount?: number }>; customSlots?: unknown };
  ...
  if (body.customSlots === undefined && (![30, 60, 120].includes(slotMinutes) || minutes(closingTime) <= minutes(openingTime))) return Response.json({ error: "営業時間または区切り時間が不正です" }, { status: 400 });
  const id = crypto.randomUUID();
  let slots: Array<{ id: string; planId: string; date: string; startTime: string; endTime: string; requiredCount: number; role: string }>;
  let effectiveOpeningTime = openingTime;
  let effectiveClosingTime = closingTime;
  if (body.customSlots !== undefined) {
    try { slots = parseCustomSlots(body.customSlots, id, startDate, endDate); } catch (error) { return Response.json({ error: error instanceof Error ? error.message : "カスタム入力を確認してください" }, { status: 400 }); }
    effectiveOpeningTime = slots.reduce((min, slot) => slot.startTime < min ? slot.startTime : min, slots[0].startTime);
    effectiveClosingTime = slots.reduce((max, slot) => slot.endTime > max ? slot.endTime : max, slots[0].endTime);
  } else {
    if (rules.length === 0) return Response.json({ error: "担当・ポジションと必要人数を1つ以上設定してください" }, { status: 400 });
    slots = dateKeys(startDate, endDate).flatMap((date) => { const rows: Array<{ id: string; planId: string; date: string; startTime: string; endTime: string; requiredCount: number; role: string }> = []; for (let current = minutes(openingTime); current + slotMinutes <= minutes(closingTime); current += slotMinutes) for (const rule of rules) rows.push({ id: crypto.randomUUID(), planId: id, date, startTime: time(current), endTime: time(current + slotMinutes), requiredCount: rule.requiredCount, role: rule.role }); return rows; });
  }
  const db = getDb();
  ...
  const defaultRequiredCount = rules[0]?.requiredCount ?? slots[0]?.requiredCount ?? 1;
  await db.batch([db.insert(shiftPlans).values({ id, groupId, name, notes, startDate, endDate, openingTime: effectiveOpeningTime, closingTime: effectiveClosingTime, slotMinutes: body.customSlots === undefined ? slotMinutes : 30, defaultRequiredCount, status: "draft", createdBy: user.email }), ...slotStatements, db.insert(shiftRequestPeriods).values({ id: periodId, groupId, planId: id, name: `${name}の勤務希望`, opensOn: "", closesOn: requestCloseDate, status: "pending", createdBy: user.email })]);
  return Response.json({ plan: { id, groupId, name, notes, startDate, endDate, requestCloseDate, openingTime: effectiveOpeningTime, closingTime: effectiveClosingTime, slotMinutes: body.customSlots === undefined ? slotMinutes : 30, defaultRequiredCount, status: "draft" }, requestPeriod: { id: periodId, groupId, planId: id, name: `${name}の勤務希望`, opensOn: "", closesOn: requestCloseDate, status: "pending", createdBy: user.email }, slotCount: slots.length }, { status: 201 });
}
