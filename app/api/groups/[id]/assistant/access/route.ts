...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (payload.action === "downloadPack") {
    ...
    files["README.md"] = `# KINBAN運営支援AI 接続パック\n\nこのZIPは、${group?.name ?? groupId}（${groupId}）専用の接続情報と業務関連資料をまとめた全部入りパックです。\n\n## 接続情報\n\n- MCP URL: ${mcpUrl}\n- グループID: ${groupId}\n- APIキー: connection.envを参照\n- パッケージ版: ${assistantBusinessSet.packageVersion}\n\n## 初期設定\n\n1. このZIPを1つの作業フォルダへ展開します。\n2. Codexでは、このフォルダをプロジェクトとして開き、信頼できるプロジェクトとして確認します。\n3. Claude Codeでは、このフォルダをプロジェクトルートとして開きます。\n4. .codex/config.tomlと.mcp.jsonが同じMCP中継を起動します。中継はscripts/mcp-http-bridge.mjsからconnection.envを読み込み、KINBAN MCPへ接続します。\n5. AGENTS.mdまたはCLAUDE.mdを読んでから、tools/listとlist_groupsで対象グループへの接続を確認します。\n6. 変更前に対象、期間、権限、警告を確認します。\n\nこのZIPにはconnection.env、権限一覧、プロジェクト別MCP設定、HTTP中継、AGENTS.md、CLAUDE.md、Skill、runbook、jobs、安全資料が含まれます。connection.envとAPIキーは秘密情報です。Git、チャット、レポート、グローバル環境変数へ保存・共有しないでください。キーを使わなくなったらグループ管理から無効化してください。\n`;
