...
export async function GET(request: Request) {
  ...
  return Response.json({ data: page.map((event) => ({ ...event, attachments: fileMap.get(event.id) ?? [] })), pagination: { limit, offset, total: filtered.length, hasMore: offset + page.length < filtered.length } });
}
