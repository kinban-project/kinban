...
export default function AnnouncementsPanel({ groupId, manager = false, initialTab = "announcements", assistantUnread = false, onAssistantRead }: Props) {
  ...
  const [showAssistantUnread, setShowAssistantUnread] = useState(assistantUnread);
  ...
  useEffect(() => { setTab(initialTab); setShowAssistantUnread(assistantUnread); if (initialTab === "assistant" && !manager) { setShowAssistantUnread(false); onAssistantRead?.(); } }, [groupId, initialTab, assistantUnread, manager]);

  function openAssistantTab() {
    setTab("assistant");
    if (!manager) {
      setShowAssistantUnread(false);
      onAssistantRead?.();
    }
  }
  ...
  return <div className="announcements-panel">
    <div className="modal-head"><div><p className="eyebrow">MESSAGES</p><h2>{tab === "assistant" ? "連絡" : "お知らせ"}{selectedGroupName ? `（${selectedGroupName}）` : ""}</h2></div></div>
    {manager && <div className="message-tabs" role="tablist"><button className={tab === "announcements" ? "active" : ""} onClick={() => setTab("announcements")} role="tab" aria-selected={tab === "announcements"}>お知らせ</button><button className={tab === "assistant" ? "active" : ""} onClick={openAssistantTab} role="tab" aria-selected={tab === "assistant"}>連絡{showAssistantUnread && <span className="assistant-unread-label">未処理</span>}</button></div>}
