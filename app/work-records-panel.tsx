...
export default function WorkRecordsPanel({ groupId, manager = false }: { groupId: string; manager?: boolean }) {
  ...
  return <section className="work-records-panel">
    ...
    {manager ? <>
      ...
      <div className="work-schedule-list">{schedule.length ? schedule.slice(0, 30).map((slot) => { const slotBreaks = slot.record ? breaksFor(slot.record.id) : []; const onBreak = slotBreaks.some((item) => !item.endedAt); const draft = slot.record ? (claimDrafts[slot.record.id] ?? { start: localDateTime(slot.record.claimedStartAt ?? slot.record.startedAt), end: localDateTime(slot.record.claimedEndAt ?? slot.record.endedAt) }) : null; return <article className="work-schedule-item" key={`${slot.id}-${slot.userEmail}`}><div><strong>{slot.date} {slot.startTime}〜{slot.endTime}</strong><span>{slot.planName}{slot.role ? ` ／ ${slot.role}` : ""}</span>{slot.record && <small className="work-clock-note">打刻：{formatTime(slot.record.startedAt)}〜{formatTime(slot.record.endedAt)}</small>}</div>{slot.record?.status === "working" ? <span className="work-review-actions"><button className="small-action" disabled={busy} onClick={() => void (onBreak ? toggleBreak(slot.record!, "break-end") : toggleBreak(slot.record!, "break-start"))}>{onBreak ? "休憩終了" : "休憩開始"}</button><button className="primary-button" disabled={busy || onBreak} onClick={() => void end(slot.record!)}>勤務終了</button></span> : slot.record ? <div className="work-schedule-claim"><span className={`work-status work-status-${slot.record.status}`}>{statusLabel(slot.record.status)}</span>{draft && <><div className="claim-time-fields"><input type="datetime-local" aria-label="申告開始" value={draft.start} disabled={slot.record.status === "approved"} onChange={(event) => setClaimDrafts((current) => ({ ...current, [slot.record!.id]: { ...draft, start: event.target.value } }))} /><span>〜</span><input type="datetime-local" aria-label="申告終了" value={draft.end} disabled={slot.record.status === "approved"} onChange={(event) => setClaimDrafts((current) => ({ ...current, [slot.record!.id]: { ...draft, end: event.target.value } }))} /></div><span className="work-review-actions">{slot.record.status !== "approved" && <><button className="small-action" disabled={busy} onClick={() => void applySchedule(slot.record!)}>シフト通り</button><button className="small-action" disabled={busy} onClick={() => void saveClaim(slot.record!)}>保存</button></>}</span></>}</div> : <button className="small-action" disabled={busy} onClick={() => void start(slot.id)}>勤務開始</button>}</article>; }) : <p className="empty-state">割り当て済みの公開シフトはありません。予定外の勤務開始も記録できます。</p>}</div>
