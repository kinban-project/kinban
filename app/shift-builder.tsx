...
type Detail = { plan: Plan; slots: Slot[]; assignments: Array<{ slotId: string; userEmail: string }>; members: Member[]; closedDates?: string[] };
type SlotRule = { role: string; requiredCount: string };

const initial = { groupId: "", name: "7月後半シフト", startDate: "2026-07-16", endDate: "2026-07-31", openingTime: "09:00", closingTime: "18:00", slotMinutes: "60" };
...
export default function ShiftBuilder({ initialGroupId }: { initialGroupId?: string }) {
  ...
  async function saveLayout() { if (!detail) return; setBusy(true); const response = await localApiFetch(`/api/shifts/${detail.plan.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ layout: { notes: detail.plan.notes, slots: detail.slots, closedDates } }) }); const data = await response.json() as { error?: string; slotCount?: number }; setNotice(response.ok ? `勤務枠を保存しました（${data.slotCount ?? detail.slots.length}枠）` : (data.error ?? "勤務枠を保存できませんでした")); if (response.ok) await openPlan(detail.plan.id); setBusy(false); }
  ...
  return <section className="shift-builder-card">
    ...
    {!detail ? <>
      <form className="shift-condition-form" onSubmit={createPlan}>
        ...
        <div className="form-row"><label>開始日<input type="date" required value={form.startDate} onChange={(event) => setForm({ ...form, startDate: event.target.value })} /></label><label>終了日<input type="date" required value={form.endDate} onChange={(event) => setForm({ ...form, endDate: event.target.value })} /></label></div>
