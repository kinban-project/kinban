...
type Plan = { id: string; groupId: string; name: string; notes: string; startDate: string; endDate: string; openingTime: string; closingTime: string; slotMinutes: number; defaultRequiredCount: number; status: "draft" | "published" };
...
export default function ShiftBuilder({ initialGroupId }: { initialGroupId?: string }) {
  ...
  async function loadGroups() {
    ...
    if (response.ok) {
      ...
      if (!form.groupId && data.groups[0]) setForm((current) => ({ ...current, groupId: data.groups[0].id }));
    }
  }

  async function loadPlans(groupId: string) {
    if (!groupId) return;
    const response = await localApiFetch(`/api/shifts?groupId=${encodeURIComponent(groupId)}`);
    if (response.ok) {
  ...
  async function openPlan(id: string) {
    ...
    if (response.ok) { const next = await response.json() as Detail; setDetail(next); setClosedDates(next.closedDates ?? []); }
  }
  ...
  useEffect(() => { if (initialGroupId) setForm((current) => ({ ...current, groupId: initialGroupId })); }, [initialGroupId]);
  ...
  function updateSlotRule(index: number, next: Partial<SlotRule>) { setSlotRules((current) => current.map((rule, ruleIndex) => ruleIndex === index ? { ...rule, ...next } : rule)); }
  function updateSlot(id: string, next: Partial<Slot>) { if (!detail) return; setDetail({ ...detail, slots: detail.slots.map((slot) => slot.id === id ? { ...slot, ...next } : slot) }); }
  function closeDate(date: string) { setClosedDates((current) => current.includes(date) ? current : [...current, date]); }
  function reopenDate(date: string) { setClosedDates((current) => current.filter((item) => item !== date)); }
  async function saveLayout(action?: "start-requests") { if (!detail) return; setBusy(true); const response = await localApiFetch(`/api/shifts/${detail.plan.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action, requestCloseDate: detail.requestPeriod?.closesOn, layout: { notes: detail.plan.notes, slots: detail.slots, closedDates } }) }); const data = await response.json() as { error?: string; slotCount?: number }; setNotice(response.ok ? (action === "start-requests" ? "シフト希望の受付を開始しました" : `勤務枠を保存しました（${data.slotCount ?? detail.slots.length}枠）`) : (data.error ?? "勤務枠を保存できませんでした")); if (response.ok) await openPlan(detail.plan.id); setBusy(false); }
  ...
  async function save(status: "draft" | "published") { if (!detail) return; setBusy(true); const response = await localApiFetch(`/api/shifts/${detail.plan.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ assignments: assignmentMap(), status }) }); const data = await response.json() as { error?: string; warnings?: string[] }; setWarnings(data.warnings ?? []); setNotice(response.ok ? (status === "published" ? "シフトを公開しました" : "下書きを保存しました") : (data.error ?? "保存できませんでした")); if (response.ok) await openPlan(detail.plan.id); setBusy(false); }

  return <section className="shift-builder-card">
    <div className="shift-builder-head"><div><p className="eyebrow">SHIFT PLANNER</p><h2>勤務枠調整</h2><p>条件から勤務枠を作り、表で調整します。</p></div>{detail && <button className="ghost-button" onClick={() => setDetail(null)}>新しい計画</button>}</div>
    {!detail ? <>
      <form className="shift-condition-form" onSubmit={createPlan}>
        <label>グループ<select required value={form.groupId} onChange={(event) => setForm({ ...form, groupId: event.target.value })}><option value="">選択してください</option>{editableGroups.map((group) => <option value={group.id} key={group.id}>{group.name}</option>)}</select></label>
      ...
      <div className="shift-actions shift-actions-top"><button className="ghost-button" onClick={() => void saveLayout()} disabled={busy}>保存</button></div>
      {detail.requestPeriod && <div className={`shift-request-state ${detail.requestPeriod.status}`}><strong>シフト希望受付</strong>{detail.requestPeriod.status === "pending" ? <label>受付期限<input type="date" value={detail.requestPeriod.closesOn} onChange={(event) => setDetail({ ...detail, requestPeriod: { ...detail.requestPeriod!, closesOn: event.target.value } })} /></label> : <span>{detail.requestPeriod.opensOn}〜{detail.requestPeriod.closesOn} ／ {detail.requestPeriod.status === "open" ? "受付中" : "締切"}</span>}</div>}
      ...
      <div className="shift-actions"><button className="ghost-button" onClick={() => void saveLayout()} disabled={busy}>保存</button>{detail.requestPeriod?.status === "pending" && <button className="primary-button" onClick={() => void saveLayout("start-requests")} disabled={busy}>希望受付開始</button>}</div><p className="shift-help">人数・時間・担当を調整できます。休業にした日は、勤務枠を削除して保存します。受付開始後は受付期限を変更できません。</p>
    </>}
