    ...
    {selected && <div className="group-detail">
      ...
      {isAdmin && <section className="group-rules-panel">
        ...
        <div className="labor-rule-settings">
          ...
          <div className="labor-rule-list">
            <label className="labor-rule-row"><input type="checkbox" checked={selected.group.laborPlannedBreakWarning !== false} onChange={(event) => void updateGroupRules({ laborPlannedBreakWarning: event.target.checked })} /><strong>予定休憩の確認</strong><span>6時間超：45分／8時間超：60分</span></label>
            <label className="labor-rule-row"><input type="checkbox" checked={selected.group.laborDailyHoursWarning !== false} onChange={(event) => void updateGroupRules({ laborDailyHoursWarning: event.target.checked })} /><strong>1日の実働上限</strong><input type="number" min="1" value={Math.round((selected.group.laborDailyHoursLimitMinutes ?? 480) / 60)} onChange={(event) => setSelected((current) => current ? { ...current, group: { ...current.group, laborDailyHoursLimitMinutes: Number(event.target.value) * 60 } } : current)} onBlur={(event) => void updateGroupRules({ laborDailyHoursLimitMinutes: Number(event.currentTarget.value) * 60 })} /><span>時間</span></label>
            <label className="labor-rule-row"><input type="checkbox" checked={selected.group.laborWeeklyHoursWarning !== false} onChange={(event) => void updateGroupRules({ laborWeeklyHoursWarning: event.target.checked })} /><strong>週の実働上限</strong><input type="number" min="1" value={Math.round((selected.group.laborWeeklyHoursLimitMinutes ?? 2400) / 60)} onChange={(event) => setSelected((current) => current ? { ...current, group: { ...current.group, laborWeeklyHoursLimitMinutes: Number(event.target.value) * 60 } } : current)} onBlur={(event) => void updateGroupRules({ laborWeeklyHoursLimitMinutes: Number(event.currentTarget.value) * 60 })} /><span>時間</span></label>
            <label className="labor-rule-row"><input type="checkbox" checked={selected.group.laborRestIntervalWarning !== false} onChange={(event) => void updateGroupRules({ laborRestIntervalWarning: event.target.checked })} /><strong>勤務間インターバル</strong><input type="number" min="0" value={Math.round((selected.group.laborRestIntervalMinutes ?? 660) / 60)} onChange={(event) => setSelected((current) => current ? { ...current, group: { ...current.group, laborRestIntervalMinutes: Number(event.target.value) * 60 } } : current)} onBlur={(event) => void updateGroupRules({ laborRestIntervalMinutes: Number(event.currentTarget.value) * 60 })} /><span>時間以上</span></label>
            <label className="labor-rule-row"><input type="checkbox" checked={selected.group.laborConsecutiveDaysWarning !== false} onChange={(event) => void updateGroupRules({ laborConsecutiveDaysWarning: event.target.checked })} /><strong>連続勤務日数</strong><input type="number" min="1" value={selected.group.laborConsecutiveDaysLimit ?? 7} onChange={(event) => setSelected((current) => current ? { ...current, group: { ...current.group, laborConsecutiveDaysLimit: Number(event.target.value) } } : current)} onBlur={(event) => void updateGroupRules({ laborConsecutiveDaysLimit: Number(event.currentTarget.value) })} /><span>日以上</span></label>
            <label className="labor-rule-row"><input type="checkbox" checked={selected.group.laborWeeklyRestWarning !== false} onChange={(event) => void updateGroupRules({ laborWeeklyRestWarning: event.target.checked })} /><strong>休日数</strong><span>7日間：</span><input type="number" min="0" value={selected.group.laborWeeklyRestDaysRequired ?? 1} onChange={(event) => setSelected((current) => current ? { ...current, group: { ...current.group, laborWeeklyRestDaysRequired: Number(event.target.value) } } : current)} onBlur={(event) => void updateGroupRules({ laborWeeklyRestDaysRequired: Number(event.currentTarget.value) })} /><span>日以上　28日間：</span><input type="number" min="0" value={selected.group.laborFourWeekRestDaysRequired ?? 4} onChange={(event) => setSelected((current) => current ? { ...current, group: { ...current.group, laborFourWeekRestDaysRequired: Number(event.target.value) } } : current)} onBlur={(event) => void updateGroupRules({ laborFourWeekRestDaysRequired: Number(event.currentTarget.value) })} /><span>日以上</span></label>
          </div>
          <div className="labor-rule-values">
            ...
            <label>連続勤務<input type="number" min="1" value={selected.group.laborConsecutiveDaysLimit ?? 7} onChange={(event) => setSelected((current) => current ? { ...current, group: { ...current.group, laborConsecutiveDaysLimit: Number(event.target.value) } } : current)} onBlur={(event) => void updateGroupRules({ laborConsecutiveDaysLimit: Number(event.currentTarget.value) })} /><span>日</span></label>
