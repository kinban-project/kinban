...
export default function GroupsPanel({ onChanged, initialGroupId }: { onChanged: () => void; initialGroupId?: string }) {
  ...
  return <section className="groups-card">
    ...
    {selected && <div className="group-detail">
      <div className="modal-head"><div><p className="eyebrow">GROUP</p><h3>{selected.group.name}</h3><small>{selected.group.id}</small></div></div>
      ...
      <h4>メンバー</h4><div className="member-cards">{selected.members.map((member) => <article className={`member-card ${member.status === "inactive" ? "is-inactive" : ""}`} key={member.userEmail}>
        <div className="member-card-head"><div><strong>{member.displayName?.trim() || member.userEmail.split("@")[0]}</strong><small>{member.userEmail}</small></div><div className="member-card-badges"><span className="member-role-badge">{roleLabels[member.role] ?? member.role}</span>{member.status === "inactive" && <span className="member-status-badge inactive">利用停止</span>}{selected.membership.role === "owner" && member.userEmail !== selected.group.ownerEmail && <select className="member-role-select" value={member.role} onChange={(event) => void updateMember({ userEmail: member.userEmail, role: event.target.value })} aria-label={`${member.displayName?.trim() || member.userEmail}の権限`}><option value="member">メンバー</option><option value="editor">管理者</option></select>}</div></div>
